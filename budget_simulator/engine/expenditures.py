...
- ``π_idx,t`` : indexation mixte — ``INDEXATION_BASELINE_RATIO`` (~54 %)
...
``INDEXATION_BASELINE_RATIO``. ``self.deflateur_cumule`` n'est PLUS
...
from ..constants import INDEXATION_BASELINE_RATIO
...
class ExpendituresMixin:
    ...
    def calculate_expenditures(self, gdp: float, inflation: float, inflation_prev: float,
        ...
        pi_idx = (INDEXATION_BASELINE_RATIO * inflation_prev
                  + (1 - INDEXATION_BASELINE_RATIO) * inflation)

        # === 3) Récurrence chaînée sur le niveau organique précédent ===
        nominal_spending = (self.depenses_primaires_precedentes
                            * (1 + g_vol) * (1 + pi_idx))

        # === 4) Plafonds en ratio PIB (garde-fous macro, inchangés) ===
